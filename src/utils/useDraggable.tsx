...
import { DraggableItemsProps } from '../types/useDraggable'
...
export default function useDragList({
  initialItems,
  delay = 250,
  debounceMs = 200,
    ...
    if (!isAnimating) {
      ...
      for (let i in elements) {
        ...
        if (currentLoopItem.item.id !== currentTarget.item.id) {
          ...
          const { top, left, right, bottom } = currentLoopItem.node.getBoundingClientRect()

          // Checks if currentTarget and mouse position is intersecting
          if (coords.y <= bottom && coords.y >= top && coords.x <= right && coords.x >= left) {
            if (!isTransitioning) {
              ...
              // before we move it.
              const cachedOpenIndexPosition = elements[intersectedIndex].node.getBoundingClientRect()

              const direction = intersectedIndex > currentOpenIndex ? 'back' : 'forward'
